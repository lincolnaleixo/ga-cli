...
export function bootstrapConfigFromEnv(
  ...
  const missing = [
    ...
    ["GOOGLE_ANALYTICS_CLIENT_SECRET", env.GOOGLE_ANALYTICS_CLIENT_SECRET],
  ]
  ...
  const tokenUri = env.GOOGLE_ANALYTICS_TOKEN_URI?.trim() || DEFAULT_TOKEN_URI;
  let parsedTokenUri: URL;
  try {
    parsedTokenUri = new URL(tokenUri);
  } catch {
    throw new Error("GOOGLE_ANALYTICS_TOKEN_URI must be a valid HTTPS URL.");
  }
  if (
    ...
    throw new Error("GOOGLE_ANALYTICS_TOKEN_URI must use HTTPS without embedded credentials or query data.");
  }

  return {
    clientId: env.GOOGLE_ANALYTICS_CLIENT_ID!.trim(),
    clientSecret: env.GOOGLE_ANALYTICS_CLIENT_SECRET!.trim(),
    tokenUri,
  };
}
